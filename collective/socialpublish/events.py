...
def _do_social_publish(obj, settings):
    result = {}
    ...
    url_for_publish = settings.url_for_publish
    ...
    if url_for_publish:
        publish_url_obj = urlparse(url_for_publish)
        url_obj = list(urlparse(url))
        url_obj[0] = publish_url_obj[0]
        url_obj[1] = publish_url_obj[1]
    ...
    if tw_consumer_token and tw_consumer_secret and \
        ...
        tw_push(tw_consumer_token, tw_consumer_secret,
               ...
               message, url)
        result["twitter"] = "On"
    if fb_app_id and fb_privacy_setting != "--NOVALUE--" and fb_access_token:
        if fb_push_select == 'MY_WALL':
            # User's wall post
            fb_push(fb_access_token, fb_privacy_setting, message, url)
            result["facebook"] = "My Wall"
        else:
            ...
            fb_page_push(fb_access_token, fb_page_id, fb_page_access_token,
                         fb_privacy_setting, message, url)
            result["facebook"] = fb_push_select
    return result
...
def social_publish(obj, event):
    ...
    if event.transition.id not in workflow_transitions:
        return None
    result = _do_social_publish(obj, settings)
